...
const SignupScreen = ({ onSignup, onLogin }: SignupScreenProps) => {
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-[100svh] bg-natty-beige p-8 flex flex-col pt-safe pb-safe">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Léa Natty" className="w-full h-14 bg-white rounded-2xl pl-14 pr-6 font-bold border-2 border-transparent focus:border-natty-teal outline-none transition-all" />
            </div>
          </div>
          <div className="space-y-2">
      ...
      <div className="space-y-4">
        <Button onClick={onSignup} variant="primary" className="w-full h-16 text-lg">S'inscrire</Button>
        <button onClick={onLogin} className="w-full py-4 text-sm font-bold text-natty-charcoal/60">
